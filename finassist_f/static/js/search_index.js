...
// Function for searching in a dictionary using async function in INDEX PAGE
const searchFieldMain = document.getElementById('searchFieldMain');

searchFieldMain.addEventListener('input', async function() {
    let response = await fetch('/search_index?q=' + searchFieldMain.value);
    ...
    let targetTable = document.getElementById(searchFieldMain.getAttribute('data-table'));
    targetTable.innerHTML = html;

    // This is for the suggestive input to the user
    let suggetInput = document.getElementById(searchFieldMain.getAttribute('list'));
